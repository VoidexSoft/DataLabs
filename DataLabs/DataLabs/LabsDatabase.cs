...
{
    [CreateAssetMenu(fileName = "New Database", menuName = "Voidex/DataLabs/Database", order = 0)]
    public class LabsDatabase : ScriptableObject, ISerializationCallbackReceiver
    {
        public const string DataLabsDatabaseName = "DataLabsDatabase";
        ...
        [SerializeField] private List<DataLabsStaticDataGroup> m_staticGroups = new List<DataLabsStaticDataGroup>();
        ...
        [Obsolete(
            "Do not use linear string lookups on the entire database as they are very inefficient O(n). Instead, get the Key from DataEntity.GetDbKey() and pass that for O(1) lookup.")]
        ...
        public List<T> GetAll<T>() where T : DataEntity
        {
            ...
            foreach (DataEntity entity in m_dataVals)
            {
                if (entity is T)
                {
                    result.Add((T) entity);
        ...
        /// <returns></returns>
        public virtual DataLabsStaticDataGroup? GetStaticGroup<T>() where T : DataEntity
        {
            foreach (DataLabsStaticDataGroup group in m_staticGroups)
            {
                if (group.SourceType == typeof(T))
                {
                    return group;
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public virtual DataLabsStaticDataGroup GetStaticGroup(Type t)
        {
            return m_staticGroups.Single(x => x.SourceType == t);
        }

        public virtual List<DataLabsStaticDataGroup> GetAllStaticGroups()
        {
            List<DataLabsStaticDataGroup> result = new List<DataLabsStaticDataGroup>();
            foreach (DataLabsStaticDataGroup group in m_staticGroups)
            {
                if (group != null)
                {
        ...
        public virtual void SetStaticGroup(DataLabsStaticDataGroup identifier)
        {
            DataLabsStaticDataGroup? existingGroup = null;
            foreach (DataLabsStaticDataGroup group in m_staticGroups)
            {
                if (group.SourceType == identifier.SourceType)
                {
                    existingGroup = group;
                    break;
                }
            ...
            {
                m_staticGroups.Remove(existingGroup);
            }
