...
{
    internal class DataLabImportUtility : IImportUtility
    {
        ...
        public DataLabImportUtility(ImportState state)
        {
            m_state = state;
        }
        ...
        public string BuildAssetPath(string classTitle, string worksheetName, string assetExtension = ".asset", string assetDirectory = "")
        {
            ...
            string assetPathAndName = Path.Combine(DataLabsEditorUtility.GetPathToDataLabsStorageFolder(), fileName);
        ...
        public ScriptableObject FindOrCreateAsset(Type type, string path)
        {
            ...
            if (asset == null)
            {
                ...
                dataEntity.Title = DataSheets[0].Id.WorksheetName;
