...
using Voidex.DataLabs.Editor.GoogleSheets.Internal;
...
namespace Voidex.DataLabs.GoogleSheets.Editor
{
    public static class StringConverter
    {
        ...
        private static readonly Dictionary<Type, Converter> m_simpleConverters = new Dictionary<Type, Converter>()
        {
            ...
            //{typeof(LocalizedString), ToLocalizedString},
            {typeof(IResourceLocation), ToResourceLocation},
            {typeof(AssetReference), ToAssetReference},
            {typeof(AssetReferenceT<Sprite>), ToAssetReferenceT<Sprite>},
            {typeof(AssetReferenceT<Texture2D>), ToAssetReferenceT<Texture2D>},
            {typeof(AssetReferenceT<AudioClip>), ToAssetReferenceT<AudioClip>},
            {typeof(AssetReferenceGameObject), ToAssetReferenceGameObject},
            {typeof(AssetReferenceT<ScriptableObject>), ToAssetReferenceT<ScriptableObject>},
        ...
        public static bool IsSimpleConvertable(Type type)
        {
            return m_simpleConverters.ContainsKey(type);
        }
        ...
        // private static object ToLocalizedString(string value)
        // {
        //     //split the value into the reference and the table table/entry
        //     string[] split = value.Split('/');
        //     if (split.Length != 2)
        //     {
        //         return null;
        ...
        //     localizedString.TableReference = split[0];
        //     localizedString.TableEntryReference = split[1];
        //     return localizedString;
        // }
        
        private static object ToResourceLocation(string addressKey)
        {
            return addressKey;
        }

        private static object ToAssetReference(string addressKey)
        {
            return AddressablesUtils.GetAssetReferenceFromAddress(addressKey);
        }
        
        private static object ToAssetReferenceT<T>(string value) where T : UnityEngine.Object
        {
            return AddressablesUtils.GetAssetReferenceFromAddress<T>(value);
        }
        
        private static object ToAssetReferenceGameObject(string addressKey)
        {
            return AddressablesUtils.GetAssetReferenceGameObjectFromAddress(addressKey);
        }
        ...
        private static object ToSprite(string value)
        {
            ...
            if (guids != null && guids.Length > 0)
            {
                ...
                {
                    var data = AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(obj), typeof(Sprite));
                    if (data.name == value)
                    {
                        return data;
                    }
