...
namespace Voidex.DataLabs.GoogleSheets.Editor
{
    public class DataSheet
    {
        ...
        internal DataSheet(WorksheetID id, ValueRange valueRange, int frozenRows)
        {
            Id = id;

            // determine the max size of a row to apply to all rows
            int maxLength = 0;
            for (int ix = 0; ix < valueRange.Values.Count; ix++)
            {
                if ((valueRange.Values[ix]?.Count ?? 0) > maxLength)
                {
                    maxLength = valueRange.Values[ix].Count;
                }
            }

            // we need to remove unused fields (ie, the string is null or empty)
            string[] fields = valueRange.Values[Math.Max(0, frozenRows - 1)].Select(o => o.ToString()).ToArray();
            ...
            for (int ix = Math.Max(0, frozenRows - 1) + 1; ix < valueRange.Values.Count; ix++)
            {
                var e = valueRange.Values[ix].Select(o => o.ToString()).ToArray();
                ...
                CopyStringArray(e, row); // maintains original data structure
